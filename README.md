...
вынесены в `src/main/api/steps/bank_db_steps.py`. В DB-тестах остаются целевая
API-операция, ключевые ассерты полей ответа и вызов проверки БД.
Подготовку пользователей, счетов, исходного кредита и снимков состояния
выполняют типизированные фикстуры в `src/main/api/fixtures/bank_fixture.py`.
API-сценарии этих операций также получают подготовленные данные из фикстур.
Allure-декораторы описывают сценарий.
